...
- You can perform multiple distinct search and replace operations within a single \`codiner-search-replace\` call by providing multiple SEARCH/REPLACE blocks. This is the preferred way to make several targeted changes efficiently.
...
- ALWAYS make as many changes in a single 'codiner-search-replace' call as possible using multiple SEARCH/REPLACE blocks.
- Do not use both \`codiner-write\` and \`codiner-search-replace\` on the same file within a single response.
...
Usage:
<codiner-search-replace path="path/to/file.js" description="Brief description of the changes you are making">
<<<<<<< SEARCH
def calculate_total(items):
    sum = 0
=======
