...
interface CodinerEngineProviderOptions {
 ...
 * Extracts files from <codiner-read> and <codiner-code-search-result> tags.
 */
export function parseFilesFromMessage(content: string): string[] {
  ...
  // Parse <codiner-read path="$filePath"></codiner-read>
  const codinerReadRegex = /<codiner-read\s+path="([^"]+)"\s*><\/codiner-read>/gs;
  let match: RegExpExecArray | null;
  while ((match = codinerReadRegex.exec(content)) !== null) {
  ...
  // Parse <codiner-code-search-result>...</codiner-code-search-result>
  const codeSearchRegex =
    /<codiner-code-search-result>(.*?)<\/codiner-code-search-result>/gs;
...
export async function processChatMessagesWithVersionedFiles({
  ...
  for (
    ...
    const engineOptions = message.providerOptions?.[
      "codiner-engine"
    ] as unknown as CodinerEngineProviderOptions;
