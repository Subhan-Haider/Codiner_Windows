...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  {
    ...
    description:
      "Installs the Codiner component tagger Vite plugin and its dependencies.",
    manualUpgradeUrl: "https://codiner.online/docs/upgrades/select-component",
  },
  {
    id: "capacitor",
...
function isComponentTaggerUpgradeNeeded(appPath: string): boolean {
  ...
  try {
    ...
    return !viteConfigContent.includes("@codiner-sh/react-vite-component-tagger");
  } catch (e) {
...
async function applyComponentTagger(appPath: string) {
  ...
  if (content.includes("plugins: [")) {
    if (!content.includes("codinerComponentTagger()")) {
      content = content.replace(
        "plugins: [",
        "plugins: [codinerComponentTagger(), ",
      );
    }
  } else {
    throw new Error(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const process = spawn(
      "pnpm add -D @codiner-sh/react-vite-component-tagger || npm install --save-dev --legacy-peer-deps @codiner-sh/react-vite-component-tagger",
      {
        cwd: appPath,
        shell: true,
        stdio: "pipe",
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[codiner] add Codiner component tagger",
    });
