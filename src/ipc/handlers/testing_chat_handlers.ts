...
const TEST_RESPONSES: Record<string, string> = {
  ...
  <codiner-add-dependency packages="deno"></codiner-add-dependency>
  
  EOM`,
  "add-non-existing-dep": `I'll add that dependency for you.
  
  <codiner-add-dependency packages="@angular/does-not-exist"></codiner-add-dependency>
  
  EOM`,
  "add-multiple-deps": `I'll add that dependency for you.
  
  <codiner-add-dependency packages="react-router-dom react-query"></codiner-add-dependency>
  
  EOM`,
  write: `Hello world
  <codiner-write path="src/hello.ts" content="Hello world">
...
export function getTestResponse(prompt: string): string | null {
  const match = prompt.match(/\[codiner-qa=([^\]]+)\]/);
  if (match) {
