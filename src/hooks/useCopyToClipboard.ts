...
export const useCopyToClipboard = () => {
  ...
  const copyMessageContent = async (messageContent: string) => {
    try {
      // Use the same parsing logic as CodinerMarkdownParser but convert to clean text
      const formattedContent = convertCodinerContentToMarkdown(messageContent);
  ...
  // Convert Codiner content to clean markdown using the same parsing logic as CodinerMarkdownParser
  const convertCodinerContentToMarkdown = (content: string): string => {
    if (!content) return "";

    // Use the same parsing functions from CodinerMarkdownParser
  ...
  // Convert individual custom tags to markdown (reuse the same logic from CodinerMarkdownParser)
  ...
  // Reuse the same parsing functions from CodinerMarkdownParser but simplified
