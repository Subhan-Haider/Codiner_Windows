...
import { VanillaMarkdownParser } from "./CodinerMarkdownParser";
import { CustomTagState } from "./stateTypes";
import { CodinerTokenSavings } from "./CodinerTokenSavings";

interface CodinerThinkProps {
  node?: any;
  children?: React.ReactNode;
}

export const CodinerThink: React.FC<CodinerThinkProps> = ({ children, node }) => {
  ...
  const tokenSavingsMatch =
    typeof children === "string"
      ? children.match(
        /^Codiner-token-savings\?original-tokens=([0-9.]+)&smart-context-tokens=([0-9.]+)$/,
      )
      : null;
  ...
  // If it's token savings format, render CodinerTokenSavings component
