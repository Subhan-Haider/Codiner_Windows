  ...
  ({ className, children, node, ...props }: CodeHighlightProps) => {
    ...
    return !isInline ? (
      ...
      >
        {language ? (
          <>
            <div className="absolute top-2 left-3 text-xs tracking-tighter text-muted-foreground/85 select-none pointer-events-none">
              {language}
            </div>
            {code && (
              <button
                className="absolute top-2 right-2 flex items-center text-xs cursor-pointer bg-background/40 hover:bg-background/80 p-1.5 rounded transition-all backdrop-blur-sm border border-transparent hover:border-border/50"
                onClick={handleCopy}
                type="button"
              >
