...
function buildTestSettings(
  ...
  if (overrides.enableCodinerPro && overrides.hasApiKey !== false) {
    return {
      ...baseSettings,
      enableCodinerPro: true,
      providerSettings: {
        auto: {
...
vi.mock("@/paths/paths", () => ({
  getCodinerAppPath: vi.fn((appPath: string) => `/mock/apps/${appPath}`),
}));
...
describe("handleLocalAgentStream", () => {
  ...
  describe("Pro status validation", () => {
    it("should send error when Codiner Pro is not enabled", async () => {
      // Arrange
      const { event, getMessagesByChannel } = createFakeEvent();
      mockSettings = buildTestSettings({ enableCodinerPro: false });
      ...
      expect(errorMessages[0].args[0]).toMatchObject({
        chatId: 1,
        error: expect.stringContaining("Agent v2 requires Codiner Pro"),
      });
    });
  ...
  describe("Chat lookup", () => {
    it("should throw error when chat is not found", async () => {
      ...
      mockSettings = buildTestSettings({ enableCodinerPro: true });
