...
describe("parseFilesFromMessage", () => {
  describe("codiner-read tags", () => {
    it("should parse a single codiner-read tag", () => {
      const input = '<codiner-read path="src/components/Button.tsx"></codiner-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/Button.tsx"]);
    });

    it("should parse multiple codiner-read tags", () => {
      const input = `
        <codiner-read path="src/components/Button.tsx"></codiner-read>
        <codiner-read path="src/utils/helpers.ts"></codiner-read>
        <codiner-read path="src/styles/main.css"></codiner-read>
      `;
      const result = parseFilesFromMessage(input);
    ...
    it("should trim whitespace from file paths in codiner-read tags", () => {
      const input =
        '<codiner-read path="  src/components/Button.tsx  "></codiner-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/Button.tsx"]);
    ...
    it("should skip empty path attributes", () => {
      const input = `
        <codiner-read path="src/components/Button.tsx"></codiner-read>
        <codiner-read path=""></codiner-read>
        <codiner-read path="src/utils/helpers.ts"></codiner-read>
      `;
      const result = parseFilesFromMessage(input);
      expect(result).toEqual([
    ...
    it("should handle file paths with special characters", () => {
      const input =
        '<codiner-read path="src/components/@special/Button-v2.tsx"></codiner-read>';
      const result = parseFilesFromMessage(input);
      expect(result).toEqual(["src/components/@special/Button-v2.tsx"]);
    });
  });

  describe("codiner-code-search-result tags", () => {
    it("should parse a single file from codiner-code-search-result", () => {
      const input = `<codiner-code-search-result>
src/components/Button.tsx
    ...
    it("should parse multiple files from codiner-code-search-result", () => {
      const input = `<codiner-code-search-result>
src/components/Button.tsx
...
src/utils/helpers.ts
    ...
    it("should skip empty lines in codiner-code-search-result", () => {
      const input = `<codiner-code-search-result>
src/components/Button.tsx
...
src/utils/helpers.ts
    ...
    it("should handle multiple codiner-code-search-result tags", () => {
      const input = `<codiner-code-search-result>
src/components/Button.tsx
src/components/Input.tsx
</codiner-code-search-result>
...
src/styles/main.css
  ...
  describe("mixed tags", () => {
    it("should parse both codiner-read and codiner-code-search-result tags", () => {
      const input = `
<codiner-read path="src/config/app.ts"></codiner-read>
...
<codiner-read path="src/components/Header.tsx"></codiner-read>
...
<codiner-read path="src/config/site.ts"></codiner-read>
...
`;
  ...
  describe("edge cases", () => {
    ...
    it("should handle malformed tags gracefully", () => {
      const input = `
<codiner-read path="src/file1.ts"
<codiner-code-search-result>
src/file2.ts
`;
      const result = parseFilesFromMessage(input);
    ...
    it("should handle nested angle brackets in file paths", () => {
      const input =
        '<codiner-read path="src/components/Generic<T>.tsx"></codiner-read>';
      const result = parseFilesFromMessage(input);
...
SRC/COMPONENTS/BUTTON.TSX
    ...
    it("should handle very long file paths", () => {
      ...
      const input = `<codiner-read path="${longPath}"></codiner-read>`;
      const result = parseFilesFromMessage(input);
...
describe("processChatMessagesWithVersionedFiles", () => {
  ...
  describe("processing assistant messages", () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'I found this file: <codiner-read path="src/old.ts"></codiner-read>',
          providerOptions: {
            "codiner-engine": {
      ...
      const chatMessages: ModelMessage[] = [
        {
          ...
          content: [
            {
              type: "text",
              text: 'Here is the file: <codiner-read path="src/array.ts"></codiner-read>',
            },
            {
              type: "text",
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "user",
          content:
            'Check this: <codiner-read path="src/user-file.ts"></codiner-read>',
        },
      ];
      const appPath = "/test/app";
    ...
    it("should skip assistant messages without sourceCommitHash", async () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: 'File here: <codiner-read path="src/no-commit.ts"></codiner-read>',
          // No providerOptions
        },
        {
          role: "assistant",
          content:
            'Another file: <codiner-read path="src/no-commit2.ts"></codiner-read>',
          providerOptions: {
            // codiner-engine not set
          },
        },
      ];
      const appPath = "/test/app";
  ...
  describe("parsing multiple file paths", () => {
    it("should process multiple files from codiner-code-search-result", async () => {
      const { getFileAtCommit } = await import("@/ipc/utils/git_utils");
      const mockGetFileAtCommit = vi.mocked(getFileAtCommit);
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: `<codiner-code-search-result>
src/file1.ts
src/file2.ts
</codiner-code-search-result>`,
          providerOptions: {
    ...
    it("should process mixed codiner-read and codiner-code-search-result tags", async () => {
      const { getFileAtCommit } = await import("@/ipc/utils/git_utils");
      const mockGetFileAtCommit = vi.mocked(getFileAtCommit);
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: `
<codiner-read path="src/file1.ts"></codiner-read>
  ...
  describe("error handling", () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content:
            'Missing file: <codiner-read path="src/missing.ts"></codiner-read>',
          providerOptions: {
            "codiner-engine": {
              sourceCommitHash: "commit1",
            },
    ...
    it("should handle getFileAtCommit throwing an error", async () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: 'Error file: <codiner-read path="src/error.ts"></codiner-read>',
          providerOptions: {
            "codiner-engine": {
              sourceCommitHash: "commit1",
            },
  ...
  describe("multiple messages", () => {
    it("should process multiple messages with different commits", async () => {
      ...
      const chatMessages: ModelMessage[] = [
        ...
        {
          role: "assistant",
          content: 'Here it is: <codiner-read path="src/file1.ts"></codiner-read>',
          providerOptions: {
            "codiner-engine": {
              sourceCommitHash: "commit1",
            },
        ...
        },
        {
          role: "assistant",
          content:
            'Here it is again: <codiner-read path="src/file1.ts"></codiner-read>',
          providerOptions: {
            "codiner-engine": {
  ...
  describe("integration with files parameter", () => {
    it("should combine files parameter with versioned files from messages", async () => {
      ...
      const chatMessages: ModelMessage[] = [
        {
          role: "assistant",
          content: 'Old version: <codiner-read path="src/old.ts"></codiner-read>',
          providerOptions: {
            "codiner-engine": {
              sourceCommitHash: "abc123",
            },
